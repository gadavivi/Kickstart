...
describe('Campaigns', () => {
    it('deploys a factory and a capmpaign', () => {
    ...
    it('allows people to contribute money and mark them as approvers', async () => {
        ...
        const isContributer = await campaign.methods.apporvers(accounts[1]).call();
        assert(isContributer);
    })

    it('minimum counribution.', async () => {
        try {
            await campaign.methods.contribute().send( {
                value: '5',
                from: accounts[1]
    ...
    it('allow a manager to make a payment request.', async () => {
        ...
        .createRequest('Buy baterries', '100', accounts[1])
        .send({
            from: accounts[0],
            gas: '1000000'
        });
        ...
        assert.equal('Buy baterries', request.description);
    });
